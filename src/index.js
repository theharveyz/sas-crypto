...
export default class SAS {
  ...
  encrypt(data) {
    ...
    // 加密aesKey
    aesKey = ursa.isPrivateKey(this.rsakey) ?
      this.privateRsaEncrypt(aesKey) : this.publicRsaEncrypt(aesKey);
  ...
  decrypt(encryptedData) {
    ...
    const aesKey = ursa.isPrivateKey(this.rsakey) ?
      this.privateRsaDecrypt(data[0]) : this.publicRsaDecrypt(data[0]);
  ...
  aesDecrypt(key, data) {
    ...
    let decryptedData = '';
    decryptedData += cip.update(data, 'base64', 'utf8');
    decryptedData += cip.final('utf8');
